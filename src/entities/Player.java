	...
	public void tick(Block[][] b, ArrayList<MovingBlock> movingBlocks) {
		...
		//conversion de variables double a int para poder ser trabajadas
			...
			for(int i=0; i<movingBlocks.size();i++) {
				...
				if(movingBlocks.get(i).getID()!= 0&&movingBlocks.get(i).getID()!= 2/*&&movingBlocks.get(i).getID()!= 11*/) {
					//colision derecha
					if(Collision.playerMovingBlock(new Point(iX+width+(int) GameState.xOffset,iY+ (int)GameState.yOffset+2), movingBlocks.get(i))
						...
						right=false;
						if(movingBlocks.get(i).getID()== 11) {isDead=true;}
					}
					...
					for(int j =0;j<b[0].length;j++){
						...
						if(b[i][j].getID()==2) {
							if(Collision.playerBlock(new Point(iX+(int)GameState.xOffset+1,iY+(int)GameState.yOffset),b[i][j])
									||Collision.playerBlock(new Point(iX+width+(int)GameState.xOffset-2,iY+(int)GameState.yOffset), b[i][j])) {
								isDead=true;
								System.out.println("Jorge"+deadOrAlive());
		...
		for(int i =0;i<b.length;i++) {
			for(int j =0;j<b[0].length;j++){
				...
				if(b[i][j].getID()==4) {
					if(Collision.playerBlock(new Point(iX+(int)GameState.xOffset,iY+(int)GameState.yOffset),b[i][j])
							||Collision.playerBlock(new Point(iX+width+(int)GameState.xOffset,iY+(int)GameState.yOffset), b[i][j])) {
						win=true;
						System.out.println("Jorge"+winGame());
		...
		for(int i =0;i<b.length;i++) {
			for(int j =0;j<b[0].length;j++){
				...
				if(b[i][j].getID()==5) {
					if(Collision.playerBlock(new Point(iX + (int) GameState.xOffset, iY - 1 + (int) GameState.yOffset), b[i][j])
		                    || Collision.playerBlock(new Point(iX + width + (int) GameState.xOffset, iY + 2 + (int) GameState.yOffset), b[i][j])) {
						win=true;
						System.out.println("Jorge"+winGame());
...
if(falling) {
	if(lado==0) {Images.direction="cayendoD";}
	if(lado==1) {Images.direction="cayendoI";}
